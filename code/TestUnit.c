...
extern void line1_(double* time, int* deviceEnable, double* port1, double* port2, double* tripSignal);
extern void bus2_(double* time, int* deviceEnable, double* tripSignal);
...
int main()
{
    ...
    for (i = 0; i < ROWSIZE; i++) {
        ...
        line1_(&timeData[i], &deviceEnable[i], port1[i], port2[i], tripSignal[i]);

        bus2_(&timeData[i], &deviceEnable[i], tripSignal[i]);
    }
}
